...
namespace CMF
{
	...
	public class CameraController : MonoBehaviour 
	{
		...
		[SerializeField] Vector2Reference lookInput;
		[SerializeField] FloatReference lookSensitivity;
		[SerializeField] BoolReference invertLook;
		...
		void Awake () {
			...
			if(lookInput == null)
				Debug.LogWarning("No camera look input has been attached to this gameobject", this.gameObject);
		...
		{
			_lookSensitivity = lookSensitivity.Value;
        }

		//Get user input and handle camera rotation;
		protected void HandleCameraRotation()
		{
			if(lookInput == null || Time.timeScale < 0.0001f)
				return;

			//Get input values;
			float _inputHorizontal = lookInput.Value.x * lookSensitivity.Value;
			float _inputVertical = -lookInput.Value.y * lookSensitivity.Value;

			if(invertLook.Value)
			{
				_inputHorizontal = -_inputHorizontal;
                _inputVertical = -_inputVertical;
			}
		...
		protected void RotateCamera(float _newHorizontalInput, float _newVerticalInput)
		{
			...
			currentXAngle += oldVerticalInput * Time.deltaTime;
			currentYAngle += oldHorizontalInput * Time.deltaTime;
        ...
        //    Quaternion localRotation = tr.parent != null
        //        ? Quaternion.Inverse(tr.parent.rotation) * rotation
        //        : rotation;
        ...
        //    euler.x = Mathf.Clamp(euler.x, -upperVerticalLimit, lowerVerticalLimit);
