...
namespace CMF
{
	...
	public class SmoothPosition : MonoBehaviour {
        ...
        void SmoothFixedUpdate()
        {
            //Smooth current position;
            currentPosition = SmoothFixed(currentPosition, target.position, lerpSpeed);
        ...
        Vector3 SmoothFixed(Vector3 _start, Vector3 _target, float _smoothTime)
        {
            //Convert local position offset to world coordinates;
            Vector3 _offset = tr.localToWorldMatrix * localPositionOffset;
            ...
            {
                case SmoothType.Lerp:
                    return Vector3.Lerp(_start, _target, Time.fixedDeltaTime * _smoothTime);
                case SmoothType.SmoothDamp:
                    return Vector3.SmoothDamp(_start, _target, ref refVelocity, smoothDampTime);
                default:
                    return Vector3.zero;
		...
		{
			return localPositionOffset.y;
		}

		public void SetLocalHeight(float height)
		{
			localPositionOffset = new Vector3(localPositionOffset.x, height, localPositionOffset.z);
		}
